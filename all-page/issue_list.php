...
$offset=($page-1)*$limit;
 ...
 $sql="SELECT * FROM `issue` ORDER BY  `id` DESC LIMIT $offset,$limit";
...
if(isset($_POST['search_submit'])){
     ...
     if($input!=""){
        ...
        $sql_id="SELECT * FROM `issue` WHERE `issue` like '%$input%'";
        $query_id=mysqli_query($conn,$sql_id);
        while($res_id=mysqli_fetch_array($query_id)){
          $res_name=$res_id['id'];
        
          $sql="SELECT * FROM `issue` WHERE `id`='$res_name'";
          $query=mysqli_query($conn,$sql);
          $num=mysqli_num_rows($query);
            if($num<=0){
            $_SESSION['msg2']="Data Not Found";
      ...
      $offset=($page-1)*$limit;
       ...
       // if only department input is submitted and company input is blank
        ...
        if ($select_department!=""  && $company_select=="") {
            ...
            $sql="SELECT * FROM `issue` WHERE `department`='$select_department'";
            $query=mysqli_query($conn,$sql);
            $num=mysqli_num_rows($query);
            if($num<=0){
              $_SESSION['msg2']="Data Not Found";
        ...
        elseif ($select_department==""  && $company_select!="") {
            $sql="SELECT * FROM `issue` WHERE `company_id`='$company_select'";
            $query=mysqli_query($conn,$sql);
            $num=mysqli_num_rows($query);
            if($num<=0){
              $_SESSION['msg2']="Data Not Found";
        ...
        elseif($select_department!=""  && $company_select!=""){
            ...
            $sql="SELECT * FROM `issue` WHERE  `department`='$select_department' and `company_id`='$company_select'";
            $query=mysqli_query($conn,$sql);
            $num=mysqli_num_rows($query);
            if($num<=0){
              $_SESSION['msg2']="Data Not Found";
...
?>
            <div class="container col-md-12 text-dark border border-3 rounded-4" style="height:100%; border-radius: 15px; background-color:  #f5f5dc;">
               <div class="row bg-secondary my-1 col-md-12" style="width:100%;">
                  <div class="col-md-8">
                    <h3 class="text-center text-light my-2">All Issue List</h3>
                  </div>
                  ...
                  <div class="col-md-2">
                    <a href="add_new_issue.php" class="btn btn-primary my-2" style="font-size: 15px;"><i class="fa fa-user-plus mx-1"></i>Add New Issue</a>
                  </div>

                  
                   <div class="col-md-2">
                    <a href="issue_list.php" class="btn btn-info my-2" style="font-size: 15px;"><i class="fa fa-refresh mx-1"></i>Refresh List</a>
                  </div>
                ...
                <div class="col-md-12 d-flex">
                  ...
                  <div class=" col-md-6 my-2">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                          <li class="page-item">
                            <a class="page-link" href="issue_list.php?page=<?php echo 1;?>" aria-label="Previous">
                              <span aria-hidden="true">&laquo;</span>First
                            </a>
                          </li>
                           <?php if($p>1){ ?>
                          <li class="page-item">
                            <a class="page-link" href="issue_list.php?page=<?php echo $prev; ?>">
                             Previous
                            </a>
                          </li><?php } ?>
                          ...
                          <?php
                           ...
                           if($p<=$per_page){
                            ?>
                               ...
                               <li class="page-item"><a class="page-link"  href="issue_list.php?page=<?php echo $p; ?>"><?php echo $p;  ?></a></li>
                              <?php
                                  }
                              ...
                              <?php if($p<=$per_page){ ?>
                          <li class="page-item">
                            <a class="page-link" href="issue_list.php?page=<?php echo $next; ?>">
                              Next
                            </a>
                          </li> <?php } ?>
                          <li class="page-item">
                            <a class="page-link" href="issue_list.php?page=<?php echo $per_page; ?>" aria-label="Next">
